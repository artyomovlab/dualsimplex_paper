...
lo2$set_data(data_raw, gene_anno, sample_anno, sinkhorn_iterations = 50)
...
```
```{r}
lo2$add_knn_distances_anno(annotation_names_list = c("MT"))
```
...
```{r}
cur_data <-  lo2$get_data()
new_data <- threshold_filter(cur_data, "log_mad", 0.1, genes = T, keep_lower = F)
```
