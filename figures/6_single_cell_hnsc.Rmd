...
Here we tak only 3000 most variable genes to make caluclations faster. 
...
Since we can not plot 16 dimensions, we will use UMAP for visualization purposes. It seems that UMAP is able to catch this spatial distribution of simplexes. (corner points on UMAP are usually corners in original space)
...
Once we got some projection we now are insterested how good this projeciton is. Should we remove poitns or not.
Here we combined several plots to look at
- Histograms of zero/plain distance for both genes and samples
Here we can see if some points are way to farm from the zero than others or they just don't fit into K-dimensional space. 
Such points are usually outliers and we want to remove them
- Plane/Zero disatance dimplot
This is how to look into these number jontly. Point which is just far from the zero could be a corner of a simplex.
But point which is far zero and also far from the hypotetical simplex is considered as outlier which ruins projection by pulling all the hyperplane to this point.
...
Here we remove some predefined "bad" genes. These are non-coding, mito and house keeeping genes.
In general we have as set of regular expression filters for gene names.
...
Repeat umap with new data
```{r, fig.width=12, fig.height=6}
dso$run_umap(neighbors_X = 20, neighbors_Omega = 15)
dso$plot_projection_diagnostics()
```

####  Filtering step 2. Distance filter for samples
remove sample points  with distance from zero higher than 0.16
```{r}
dso$distance_filter(zero_d_lt = 0.16, genes = F) # apply distance filter
dso$project(K)
```
...
####  Filtering step 3. Distance filter for samples
remove sample points  with distance from zero higher than 0.15
```{r}
dso$distance_filter(zero_d_lt = 0.15, genes = F) # apply distance filter
dso$project(K)
```
...
#### Filtering step 4. Distance filter for genes (zero distance)
...
dso$plot_projection_diagnostics()
```
#### Filtering step 5. Distance filter for genes (zero distance)
...
#### Filtering step 6. Distance filter for samples (plane and zero distance)

```{r}
dso$distance_filter(zero_d_lt = 0.1, plane_d_lt = 0.2, genes = F)
dso$project(K)
```
...
Just print all the steps you done
```{r}
dso$get_filtering_stats()
```

See what we removed on original Seurat UMAP
