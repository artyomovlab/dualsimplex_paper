---
title: "Figures 6C: Preprocessing of TCGA-HNSC for Linseed2"
...
## Data
```{r}
data_raw <- as.matrix(readRDS("../../data/HNSC/gene_counts.Rda"))
...
## Anotation
```{r}
patient_data <- read.csv("../../data/HNSC/nationwidechildrens.org_clinical_patient_hnsc.txt", sep = "\t")
patient_data <- patient_data[3:nrow(patient_data),]
rownames(patient_data) <- patient_data$bcr_patient_barcode

sample_data <- read.csv("../../data/HNSC/nationwidechildrens.org_biospecimen_sample_hnsc.txt", sep = "\t")
...
ribosome_pathways <- names(pathways)[grep("GO.*RIBOSOM.*", names(pathways))]
mitochondrion_pathways <- names(pathways)[grep("GO.*MITOCHOND.*", names(pathways))]
```
...
lo2$plot_projected(color_genes = gene_anno$HK, use_dims = NULL)  # NULL means default, either 2:3 or UMAP if present
```
...
#  recording a filtering log entry about that action
fData(lo2$st$data)$HK_neighbours <- rownames(lo2$get_data()) %in% filter_out

lo2$plot_projected(color_genes = filter_out, use_dims = NULL)
```
...
for (cat in remove_categories) {
  show(lo2$plot_projected(color_genes = cat, use_dims = NULL, pt_size = 0.2))
}
```
...
Note that after filtering we can't plot straight away (see error message below),
...
There are distinct outlier genes and samples.
...
```{r fig.width = 7, fig.height = 3}
lo2$distance_filter(zero_d_lt = 0.22, plane_d_lt = 0.4, genes = T)
lo2$project(n_ct_outliers)
lo2$plot_projection_diagnostics()
```

```{r fig.width = 7, fig.height = 3}
lo2$distance_filter(plane_d_lt = 0.2, genes = T)
lo2$project(n_ct_outliers)
lo2$plot_projection_diagnostics()
```

```{r fig.width = 7, fig.height = 3}
lo2$distance_filter(plane_d_lt = 0.04, genes = F)
lo2$project(n_ct_outliers)
lo2$plot_projection_diagnostics()
```

```{r fig.width = 7, fig.height = 3}
lo2$distance_filter(zero_d_lt = 0.06, genes = F)
lo2$project(n_ct_outliers)
lo2$plot_projection_diagnostics()
```
...
```{r fig.width = 7, fig.height = 3}
lo2$distance_filter(plane_d_lt = 0.12, genes = T)
lo2$project(n_ct_outliers)
lo2$plot_projection_diagnostics()
```
...
```{r fig.width = 7, fig.height = 3}
lo2$distance_filter(plane_d_lt = 0.09, genes = T)
lo2$project(n_ct_outliers)
lo2$plot_projection_diagnostics()
```
...
```{r}
lo2$plot_svd_history(c(1, 2, 3, 6, 12))
```
