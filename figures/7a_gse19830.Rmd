...
## Approximate
```{r}
#res <- getNonnegativeLowRankApproximationWithHMT(data_raw,  rank = n_ct, p=1, k=70 , iterations = 100)
#res <- getNonnegativeLowRankApproximationWithGN(simulated_data$noizy_V, rank=n_ct, l=1000, iterations = 100)
```
```{r}
plot(res$neg_count[1,])
```
```{r}
rns <- rownames(data_raw)
cns <- colnames(data_raw)
data_raw <- res$newX
rownames(data_raw) <- rns
colnames(data_raw) <- cns
```
...
dso$set_data(data_raw, gene_anno_lists = gene_anno)
...
```
```{r}
t <- fData(dso$get_data())
```
```{r}
dso$add_knn_distances_anno(annotation_names_list=c( "RRNA"))
```
