...
dir_to_save_fig <-  "../../output_figures/signature_based_7a/"
...
```{r}
save_dir <- "./7_partial_figure_save/GSE11058_benchmark"
...
```
## Projected picture using lo object method. (color by zero distance)


```{r}
#lo2$init_solution("marker_means", marker_list=marker_list)
lo2$plot_projected("zero_distance", "zero_distance", use_dims = list(3:4), with_solution = T)
```
...
```{r}
lo2$plot_projected("zero_distance", "zero_distance", use_dims = list(NULL,3:4), with_solution = T)
```
...
ggsave(file=filename, plot=proportions_plot, width=20, height=5, device=svg)
...
ggsave(file=filename, plot=expressions_plot, width=20, height=5, device=svg)
