...
```{r}
res_methods <- nmf(global_generated_data$V, current_K, list('lee', 'brunet', 'nsNMF'), nrun=5, .options='kt')
```

```{r}
res_halsAcc <- nmf(global_generated_data$V, current_K, hNMF::HALSacc, nrun=5, .options='tk', name="HALSacc")
res_methods[["HALSacc"]] <- res_halsAcc
res_PGNMF <- nmf(global_generated_data$V, current_K, hNMF::PGNMF, nrun=5, .options='tk', name="PGNMF")
res_methods[["PGNMF"]] <- res_PGNMF
res <- nmf(global_generated_data$V, current_K, als_nmf_algorithm, nrun=5, .options='tk', name="ALS")
res_methods[["ALS"]] <- res
```
...
```{r}
res <- nmf(global_generated_data$V, current_K, dualsimplex_nmf_algorithm, nrun=5, .options='tk', name="DualSimplex")
