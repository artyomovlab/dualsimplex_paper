...
dir_to_save_fig <-  "../out/signature_based_S5a/"
objects_save_dir <- "../out/signature_based_S5a/dso_objects"
...
```{r}
result_list <-  deconvolve_with_DualSimplex_with_markers(data_full, K = length(marker_list), marker_list = marker_list)
...
for (rep_n in seq(1, 5)) {
  ...
  result_list <- deconvolve_with_DualSimplex_with_markers(data_full, ntypes, marker_list = marker_list)
  ...
  rownames(reordered_proportions) <-  rownames(props)
  

  H_res <- reordered_proportions
