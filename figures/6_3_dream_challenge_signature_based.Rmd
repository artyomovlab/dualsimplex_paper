...
comparisons <-  list(c("DualSimplex", "Aginome-XMU"), 
                     c("DualSimplex", "xCell"), 
                     c("DualSimplex", "mitten_TDC19"), 
                     c("DualSimplex", "DualSimplex_RF"),
                     c("DualSimplex_RF", "Aginome-XMU"),
                     c("DualSimplex_RF", "mitten_TDC19"))

names(comparisons) <- c("Aginome-XMU", "xCell","mitten_TDC19")
```
...
box_plot_rmse_with_rf <-  plot_single_boxplot(total_result, 
...
box_plot_pearson_with_rf <-  plot_single_boxplot(total_result, 
                                      comparisons =comparisons,
                                      title = "Distance to original proportion rows", 
                                      metric_title = get_metric_plot_title(metric), log_scale = F, custom_colors = custom_colors) 
box_plot_pearson_with_rf

```



## Get metric values and plot without reference free



```{r}
source("../R/plotting_methods.R")
methods_to_check <-  names(per_run_results)[!(names(per_run_results) %in% ("DualSimplex_RF"))]
selected_per_run_results <- per_run_results[methods_to_check]
custom_colors <-  c("#E41A1C", "#A0CBE8FF", "#F28E2BFF", "#FFBE7DFF", "#59A14FFF", "#8CD17DFF", "#B6992DFF", "#F1CE63FF", "#499894FF", "#86BCB6FF", "#4E79A7FF", "#FF9D9AFF", "#79706EFF", "#BAB0ACFF", "#D37295FF", "#FABFD2FF", "#B07AA1FF", "#D4A6C8FF", "#9D7660FF", "#D7B5A6FF")
...
for (selected_dataset in selected_datasets) {
  current_true_proportions <-  per_dataset_proportions[[selected_dataset]]
  current_dataset_estimated_matrices <- lapply(selected_per_run_results, function(current_result) {current_result$proportions[[selected_dataset]][1:dim(current_true_proportions)[[1]],]})
  names(current_dataset_estimated_matrices) <-  names(selected_per_run_results)
  current_metric_values <- get_metric_values(current_dataset_estimated_matrices, current_true_proportions, metric = metric, per_row = T, normalize = T)
  current_metric_values$dataset <-  selected_dataset
  separate_results[[selected_dataset]] <- current_metric_values
}
...
filename <- paste0(dir_to_save_fig, "metrics_plot_with_legend_rf", ".svg")
ggsave(
  filename,
  result_plot,
  width = 12,
...
filename <- paste0(dir_to_save_fig, "metrics_plot_no_legend_rf", ".svg")
ggsave(
  filename,
  result_plot,
  width = 12,
