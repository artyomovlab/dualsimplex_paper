...
dir_to_save_fig <-  "../output_figures/GSE11058/"
...
Info about true samples is included for this dataset.
We use this info for validation.
...
lo2$set_data(data_raw, gene_anno_lists=list(CODING = PROTEIN_CODING_GENES))
# remove "RPLS", "LOC", "ORF", "SNOR", keep coding only
lo2$basic_filter( keep_true_cols = c("CODING"))
lo2$project(n_ct)
print(paste("Dim for the prefiltered data data:", toString(dim(lo2$get_data()))))
```
```{r fig.height = 5, fig.width = 11}
lo2$plot_svd(dims=1:15)
...
lo2$basic_filter(log_mad_gt = dummy_threshold, )
lo2$project(n_ct)

svd_plot_2 <- lo2$plot_svd(1:10) + theme_minimal(base_size = 8)
points_2 <-  lo2$plot_projected("black", "black",use_dims = 2:3, show_plots=F) +  theme_minimal(base_size = 8)
...
```{r fig.height = 4, fig.width=14}
distance_dist_plot_noizy <- plot_feature(lo2$get_data(), feature = "plane_distance") + theme_minimal(base_size = 15)
color = fData(lo2$get_data())$plane_distance
X_points_noizy <-  plot_projection_points(lo2$st$proj, use_dims=2:3,color=color,color_name="plane_distance", spaces = c("X"), pt_size = 2) + theme_minimal(base_size = 15)

distances_distribution <-   plot_feature_pair(lo2$get_data(), "plane_distance", "zero_distance", T, size = 0.1)
...
```{r}
lo2$distance_filter(plane_d_lt = 0.19, zero_d_lt = 0.3, genes = T)
lo2$project(n_ct)
```
...
```{r fig.height = 4, fig.width=14}
current_svd_plot <- lo2$plot_svd(1:10) + theme_minimal(base_size = 8)
current_points_plot <-  lo2$plot_projected("plane_distance", "plane_distance",use_dims = 2:3, show_plots=F) +  theme_minimal(base_size = 8)
...
```{r}
set.seed(23)
lo2$run_umap(neighbors_X = 25, neighbors_Omega = 5)
lo2$plot_projected("zero_distance", "zero_distance", use_dims = NULL)
```
...
# original space
lo2$plot_projected("zero_distance", "zero_distance", use_dims = c(3:4))
# umap visualization
lo2$plot_projected("zero_distance", "zero_distance", use_dims = NULL)
```
...
We make learning smaller for later iterations
We increase contribution of  term related to positivity constraint for later iterations
...
for  (lr_step in 1:LR_DECAY_STEPS) {
    ...
    beta_term <- original_beta_term
      for (x in 1:PARAMETERS_INCREASE_STEPS) {
        ...
        lo2$optim_solution(RUNS_EACH_STEP, 
...
```{r}
plot_errors(lo2$st$solution_proj,   variables = c(
    "deconv_error",
    "lamdba_error",
    "beta_error",
    "total_error"
...
```{r}
colnames(lo2$st$proj$X) <- c("R1", "R2", "R3", "R4")
colnames(lo2$st$proj$Omega) <- c("S1", "S2", "S3", "S4")
colnames(lo2$st$proj$umap$X)  <- c("UMAP1", "UMAP2")
colnames(lo2$st$proj$umap$Omega)  <- c("UMAP1", "UMAP2")

proj_solution_history <- lo2$plot_projected(use_dims = (2:3), wrap=F, with_legend = F)
proj_solution_history[[1]]
proj_solution_history[[2]]
```
Use UMAP and decorate it how it was done in the paper

```{r}
umap_solution_history <- lo2$plot_projected("zero_distance", "zero_distance", wrap=F, with_legend = F, use_dims = NULL)
...
lo2$plot_projected(rownames(to_plot), use_dims = (2:3))
```
...
plot_projection_points(lo2$st$proj, use_dims = NULL, spaces = c("X"), pt_size = 2, color=colors) +
...
it is hard to visualize 3D shape in 2D

```{r}
colors <-  which_marker(rownames(fData(lo2$st$data)), lo2$st$marker_genes)
plot_projection_points(lo2$st$proj, use_dims = (3:4), spaces = c("X"), pt_size = 2, color=colors) +
  scale_color_manual(values = colors_v[3:6], na.value = adjustcolor("grey70", alpha.f = 0.7)) + 
  labs(col = "Marker Cell Type", x = "UMAP1", y = "UMAP2") +
  theme_bw(base_family = "sans", base_size = 18) +
  theme(legend.position = "right", axis.ticks = element_blank(),
...
Omega <-  lo2$st$solution_proj$Omega
...
ax_settings <- list(title = '', 
                    ...
                    showticklabels = F, 
                    titlefont = font)
to_plot <- as.data.frame(lo2$st$proj$Omega)
