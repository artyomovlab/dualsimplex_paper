...
plotlist = list( X_points_noizy, distance_dist_plot_noizy)
...
```{r}
lo2$distance_filter(plane_d_lt = 0.2, zero_d_lt = NULL, genes = T)
lo2$project(n_ct)
```
...
lr_decay <- 0.5
params_increase <- 2
original_lambda_term <- 0.5  #coef_hinge_H
original_beta_term <- 0.5 #coef_hinge_W
lr_x <-  0.001
lr_omega <- 0.001
...
fig_X <- plotly::plot_ly(
  ...
  )  %>%  config(toImageButtonOptions = list(format = "svg", width = 650,
                                                              height = 600))
...
factor_rows <- factor(rep(rownames(true_proportions), each=nrows/n_ct), 
                      levels=c("IM-9", "Jurkat" , "THP-1", "Raji"))
factor_columns <- factor(rep(c(1:n_ct), each=ncols/n_ct), levels=c(1:n_ct))
marker_map <-  Heatmap(to_plot,name="W", 
               ...
               use_raster =T,
               column_title_gp = gpar(fill = colors_v[3:6], border = "black", fontsize =20),
