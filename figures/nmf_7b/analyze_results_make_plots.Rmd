...
## Randomly generated proportions result
```{r}
...
per_method_pictures <- plot_pictures_for_result(result_random$fit_objects, model_index = 10, dir_to_save_fig)
for (k in 1:current_K) {
  image(per_method_pictures$`Linseedv2 NMF`[[k]], col=grey(seq(0, 1, length = 64)), axes=0)
}
...
dev.off()
image(tf(matrix(V_matrix[, 18], nrow = size, ncol = size, byrow = TRUE)),
     col=grey(seq(0, 1, length = 256)), axes = FALSE)
```
...
distances_to_check_simplex$plot
```
