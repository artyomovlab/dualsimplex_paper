...
library(rJava)
dir_to_save_fig <- "../out/6_dream_rf/"
...
deconvolve_with_DualSimplex <-  function(mat, K,
           default_subsets_filter = TRUE,
           additional_subsets_filter = TRUE,
           top_mad_filter = TRUE) {
  ...
  ## Prepare all the gene sets we came up with
   msigdbr_df <- rbind(
  ...
  gene_anno <- list(
    ...
    RNA = rownames(data_raw)[grepl("RNA.*", rownames(data_raw))]
  )
  ##### Create object #####
  dso <- DualSimplexSolver$new()
  data_raw <-  remove_zero_rows(data_raw)
  dso$set_data(data_raw, max_sinkhorn_iterations = 300, max_dim = min(dim(data_raw)[[2]], 30), sinkhorn_tol = 1e-17, gene_anno=gene_anno)
  dso$project(K)
  print("Initial data size")
  print(dim(dso$st$data))
  
  ## Filter 1  
  if (default_subsets_filter) {
    dso$basic_filter(log_mad_gt = -Inf)
    dso$project(K)
    print(paste("After removing default subsets"))
    print(dim(dso$st$data))
  }
  ## Filter 2
  if (top_mad_filter) {
    current_data <- dso$get_data()
    current_data <-  top_filter(current_data, 'log_mad', top_n = 15000)
    current_data <- remove_zero_rows(current_data)
    dso$set_data(current_data, gene_anno=gene_anno, max_sinkhorn_iterations = 300, max_dim = dim(current_data)[[2]],sinkhorn_tol = 1e-17)
    dso$project(K)
    print(paste("After top log mad filter"))
    print(dim(dso$st$data))
  }
  ## Filter 3
  if (additional_subsets_filter) {
    remove_categories <- c("CC", "MITO", "RIBO", "HK", "RNA")
    dso$basic_filter(remove_true_cols_additional = remove_categories, 
                     remove_true_cols_default = FALSE,  # will not run default filterings
                     log_mad_gt = -Inf) # NULL in remove_true_cols_default means it will be selected by algo
    dso$project(K)
    print(paste("After removing additional subsets"))
    print(dim(dso$st$data))
  }
  ##### Optimization #####
  dso$project(K)
...
# Solve deconvolution with all methods
...
for (current_method in methods_to_check) {
  ...
  for (selected_dataset in names(per_dataset_matrix)) {
    ...
    result_list <- switch(current_method,
                                    "DualSimplex" = deconvolve_with_DualSimplex(input_matrix, K),
                                    "CAM3" = deconvolve_with_CAM3(input_matrix, K,  thres.low=0.3, thres.high=0.95, radius.thres=0.95, MG.num.thres = 20),
                                    "Linseed" = deconvolve_with_Linseed(input_matrix, K, topGenes = 8000),
                                    "TOAST" = deconvolve_with_toast(input_matrix, K, nmarker = 4000),
                                    "CellDistinguisher" = deconvolve_with_CellDistinguisher(input_matrix, K))
...
```{r}
dir_to_save_fig <- "../out/9_dream_rf/"
saveRDS(per_run_results,file = paste0(dir_to_save_fig, "total_results_dream_rf.rds"))
...
```{r}
per_run_results <- readRDS( paste0(dir_to_save_fig, "total_results_dream_rf.rds"))
...
comparisons <- lapply(methods_to_check, function(x) {return(c("DualSimplex", x))})[2:length(methods_to_check)]

# comparisons <-  list(c("DualSimplex", "Linseed"), c("DualSimplex", "TOAST"), c("DualSimplex", "CAM3"), c("DualSimplex", "CellDistinguisher"))
names(comparisons) <- methods_to_check[2:length(methods_to_check)]
```
### For all datasets
```{r}
...
custom_colors <- custom_colors[method_order]
total_result$method <- factor(total_result$method, levels=method_order)
comparisons <-  comparisons[method_order[method_order != "DualSimplex"]]
...
box_plot_rmse <-  plot_single_boxplot(total_result, 
                                      ...
                                      metric_title = get_metric_plot_title(metric), log_scale = F, test="t.test", 
...
for (selected_dataset in selected_datasets) {
  ...
  names(current_dataset_estimated_matrices) <-  names(per_run_results)
  current_metric_values <- get_metric_values(current_dataset_estimated_matrices, current_true_proportions, metric = metric, per_row = T, normalize = T)
  print(paste("current metric dims"))
  print(dim(current_metric_values))
  current_metric_values$dataset <-  selected_dataset
  separate_results[[selected_dataset]] <- current_metric_values
}
...
```{r}
write.csv(result_correlation_table, file =  paste0(dir_to_save_fig, "rf_total_correlation_table.csv"))
```

## For in vitro mixtures
```{r}
selected_datasets <- c("DS1", "DS2", "DS3", "DS4")
```
...
box_plot_rmse_in_vitro <-  plot_single_boxplot(total_result, 
                                      comparisons =comparisons,
                                      title = "Distance to original proportion rows", 
                                      metric_title = get_metric_plot_title(metric), log_scale = F, test="t.test", 
                                      custom_colors = custom_colors) 
box_plot_rmse_in_vitro
```
...
for (dataset_name in names(per_dataset_proportions)) {
  ...
  per_dataset_umap_with_solution[[dataset_name]] <- dso$plot_projected("markers", "zero_distance", use_dims = NULL, 
                                     with_history = F, 
                                     with_solution=T, 
                                     with_legend = T, 
                                     wrap=F )[[1]]  +   scale_color_manual(values=unlist(custom_colors[names(target_marker_list)]))
...
```{r}
per_dataset_umap_with_solution
```
...
filename <- paste0(dir_to_save_fig, "metrics_plot_in_vitro_with_legend", ".svg")
ggsave(
  filename,
  result_plot,
  width = 7.5,
...
filename <- paste0(dir_to_save_fig, "metrics_plot_in_vitro_no_legend", ".svg")
ggsave(
  filename,
  result_plot,
  width = 7.5,
