...
dir_to_save_fig <-  "../output_figures/GSE19830/"
...
```{r}
coding_info <- biomaRt::select(rat2302.db::rat2302.db, rownames(data_raw),keytype="SYMBOL", c("SYMBOL", "GENETYPE"))
...
lo2$basic_filter( keep_true_cols = c("CODING"), remove_true_cols_default = c())
lo2$project(n_ct)
...
lo2$plot_projected("zero_distance", "zero_distance", use_dims = list(2:3))
```
...
```{r}
lo2$distance_filter(plane_d_lt = 0.015, zero_d_lt = NULL, genes = T)
lo2$project(n_ct)
```
...
lr_decay <- 0.5
params_increase <- 2
original_lambda_term <- 10  #coef_hinge_H
...
### History of solution
```{r}
colnames(lo2$st$proj$X) <- c("R1", "R2", "R3")
colnames(lo2$st$proj$Omega) <- c("S1", "S2", "S3")
...
pltX <- proj_solution_history[[1]] + scale_fill_manual(values = colors_v[3:5]) + theme_minimal(base_size=12) + theme(legend.position="none")
filename <- paste0(dir_to_save_fig, "proj_solution_history_X", ".svg")

ggsave(file=filename, plot=pltX, width=3, height=3, device=svg)
pltOmega <-  proj_solution_history[[2]] + scale_fill_manual(values = colors_v[3:5]) 
...
plot_markers <- plot_projection_points(lo2$st$proj, use_dims = (2:3), spaces = c("X"), pt_size = 1, color=colors) +
  scale_color_manual(values = colors_v[3:5], na.value = adjustcolor("grey70", alpha.f = 0.7)) + 
...
ggsave(filename, plot_markers, width=4.5, height=3, device=svg)
...
ggsave(filename, plot_markers, width=3, height=3, device=svg)
...
nrows <-  dim(to_plot)[[1]]
ncols <-  dim(to_plot)[[2]]
factor_rows <- factor(rep(c("Liver","Brain", "Lung"), each=nrows/3), levels=c("Liver", "Brain", "Lung"))
factor_columns <- factor(rep(c("1","2", "3"), each=ncols/3), levels=c("1", "2", "3"))
marker_map <-  Heatmap(to_plot,name="W", 
               ...
               use_raster =T,
               column_title_gp = gpar(fill = colors_v[3:5], border = "black", fontsize =20),
