...
result <- getBM(attributes = c('ensembl_gene_id', 'mgi_symbol', 'gene_biotype'),
      mart = ensembl)
df <- as.data.frame(result)
df <- df[df$mgi_symbol != '',]
df <- df[df$gene_biotype == 'protein_coding',]
df <-  df[!duplicated(df[,'mgi_symbol']),]
sink("../data/gene_annotation_lists/CODING_MOUSE_ENSEMBL.txt")
cat(df$mgi_symbol)
sink()
```
...
```{r}
dso$distance_filter(plane_d_lt = 0.2, zero_d_lt = 0.3, genes = T)
dso$project(n_ct)
```
